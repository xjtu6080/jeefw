...
import com.jeefw.service.sys.ClusterService;
 ...
 */
@Controller
@RequestMapping("/sys/cluster")
public class ClusterController extends JavaEEFrameworkBaseController<Cluster> implements Constant {
	...
	public void getDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isNotBlank(filters)) {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				if (result.getString("field").equals("clusterName") && result.getString("op").equals("cn")) {
		...
		QueryResult<Cluster> queryResult = clusterService.doPaginationQuery(dict);
	
		JqGridPageView<Cluster> dictListView = new JqGridPageView<Cluster>();
		dictListView.setMaxResults(maxResults);
		List<Cluster> dictWithSubList = clusterService.queryClusterWithSubList(queryResult.getResultList());
	...
	@RequestMapping(value = "/getClusterName", method = { RequestMethod.POST, RequestMethod.GET })
	public void getClusterName(HttpServletRequest request, HttpServletResponse response) throws Exception {
	 String json="";
		List<Cluster> clusterList = clusterService.doQueryAll();
		if(clusterList.size()>0)
		{
			for(Cluster cluster:clusterList)
				json+=(cluster.getClusterName()+",");
		}
	...
	public void doSave(Cluster entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
		ExtJSBaseParameter parameter = ((ExtJSBaseParameter) entity);
		if (CMD_EDIT.equals(parameter.getCmd())) {
			clusterService.merge(entity);
		} else if (CMD_NEW.equals(parameter.getCmd())) {
			clusterService.persist(entity);
		}
	...
	public void operateDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (oper.equals("del")) {
			...
			String containerIds = request.getParameter("containerIds");
			
			Cluster dict = null;
			if (oper.equals("edit")) {
				dict = clusterService.get(Long.valueOf(id));
			}
			Cluster dictKeyDict = clusterService.getByProerties("ip", ip);
			Cluster parentDictkeyDict = clusterService.getByProerties("ip", type);
			if (StringUtils.isBlank(ip) || StringUtils.isBlank(clusterName)) {
				response.setStatus(HttpServletResponse.SC_LENGTH_REQUIRED);
				result.put("message", "请填写集群ip和集群名");
	...
	public void deleteDict(HttpServletRequest request, HttpServletResponse response, @RequestParam("ids") Long[] ids) throws IOException {
		boolean flag = clusterService.deleteByPK(ids);
		if (flag) {
