 ...
 */
@Controller
@RequestMapping("/sys/UserDefined")
public class UserDefinedController extends JavaEEFrameworkBaseController<UserDefined> implements Constant {

    @Resource
    private UserDefinedService dictService;

    // 查询字典的表格，包括分页、搜索和排序UserDefinedController
    ...
    public void getDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        QueryResult<UserDefined> queryResult = dictService.doPaginationQuery(dict);
        JqGridPageView<UserDefined> dictListView = new JqGridPageView<UserDefined>();
        dictListView.setMaxResults(maxResults);
        List<UserDefined> dictWithSubList = dictService.queryDictWithSubList(queryResult.getResultList());
    ...
    public void doSave(UserDefined entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    public void operateDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (oper.equals("del")) {
            ...
            UserDefined dict = null;
            if (oper.equals("edit")) {
                dict = dictService.get(Long.valueOf(id));
            }
            UserDefined dictKeyDict = dictService.getByProerties("dictKey", dictKey);
            UserDefined parentDictkeyDict = dictService.getByProerties("dictKey", parentDictkey);
            if (StringUtils.isBlank(dictKey) || StringUtils.isBlank(dictValue)) {
                ...
                UserDefined entity = new UserDefined();
