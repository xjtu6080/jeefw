 ...
 */
@Controller
@RequestMapping("/sys/rendertask")
public class TaskController extends JavaEEFrameworkBaseController<Task> implements Constant {
	...
	public void operateDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (oper.equals("del")) {
			...
			String renderClusterName = request.getParameter("renderClusterName");
			...
			if (StringUtils.isBlank(serverIp) || StringUtils.isBlank(serviceName)) {
				...
				entity.setRenderClusterName(renderClusterName);
				if (oper.equals("edit")) {
